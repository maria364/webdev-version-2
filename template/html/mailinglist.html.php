							...
							if ($res >0)
							{
								...
								while($row = $sql->fetch(PDO::FETCH_ASSOC)){
								 ...
								 if ($row["sendemail"] == 1){ //diladi an eisai sti mailing list, tote energopoiise mono to koumpi "oxi"
									$yes = "disabled";
									echo "<td><button class='sendemail' id='sendemail-yes' value=".$row["username"]."  onclick='SendEmail(this.value,".$row["sendemail"].")'  ".$yes.">Ναι</button><button class='sendemail' id='sendemail-no' value=".$row["username"]." onclick='NotSendEmail(this.value,".$row["sendemail"].")' >Όχι</button></td>";
									echo "</tr>";
								 
								 } else { // ean den eisai stin mailing list, tote energopoiise mono to koumpi "nai"
									$no = "disabled";
									echo "<td><button class='sendemail' id='sendemail-yes' value=".$row["username"]."  onclick='SendEmail(this.value,".$row["sendemail"].")'  >Ναι</button><button class='sendemail' id='sendemail-no' value=".$row["username"]." onclick='NotSendEmail(this.value,".$row["sendemail"].")'  ".$no.">Όχι</button></td>";
									echo "</tr>";
								 }
