	...
	//allagi kwdikou prosvasis		
		...
		$identity =  filter_var($_POST['identity'], FILTER_SANITIZE_STRING);
			...
			if($identity == 0){
				$q=1;
				//εάν ο χρήστης είναι user, άλλαξε την τιμή του πεδίου identity σε 1 και καν' τον administrator
				$sql = "UPDATE users SET identity=:identity WHERE username=:username";
				//$res = mysqli_query($conn,$sql);
				$stmt = $conn->prepare($sql);
		...
		//$identity =  filter_var($_POST['identity'], FILTER_SANITIZE_STRING);
				...
				$sql = $conn->prepare( "SELECT identity FROM users WHERE username=:username");
				$sql->bindParam(':username', $username, PDO::PARAM_STR);
				$sql->execute();
				$apot = $sql->rowCount(); //epistrefei ton arithmo twn grammwn pou exei vrei to apotelesma 
				...
				if($apot>0){
					$roww = $sql->fetch(PDO::FETCH_ASSOC);
						...
						if($roww["identity"] == 1){
							//echo $roww['identity'];
							//elegxo an exei allon admin to sistima

							$stmt = $conn->prepare( "SELECT * FROM users WHERE identity= :identity");
							$w=1;
							$stmt->bindValue(':identity', $w);
							$stmt->execute();
							...
							if($row>1){
									//echo $row;
							// 	//ean uparxoun kai alloi admin tha ektelesw tin allagi
								$r=0;
								$done = 1;
								$sql = "UPDATE users SET identity=:identity WHERE username=:username";
								//$res = mysqli_query($conn,$sql); 
		...
		echo  $select2;
			...
			if(($rowc >0) && $select2 == "yes") {
				...
				//if($select2 == "yes"){
				$t=1;
					$sql = "UPDATE users SET sendemail=:sendemail WHERE username=:username";
